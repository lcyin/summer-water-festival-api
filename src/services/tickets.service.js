...
export const purchaseTickets = async (ticketTypeRaw, quantityRaw) => {
    try {
        const requestBodyOrError = validateTicketPurchaseInput(ticketTypeRaw, quantityRaw);

        if (typeof requestBodyOrError === 'string') {
            throw new Error(requestBodyOrError);
        }

        const { ticketType: ticketTypeValidated, quantity: quantityValidated } = requestBodyOrError;
        ...
        const {
            ...
            limit,
            ticketType
        } = await checkTicketAvailability(ticketTypeValidated, quantityValidated);
        const client = await pool.connect();

        return processTicketPurchaseTransaction(client, ticketType, quantityValidated);
    } catch (error) {
        throw error;
    }
}
...
const processTicketPurchaseTransaction = async (
    ...
    try {
        ...
        const tickets = Array.from({ length: quantity }, (_, i) => buildOrder(orderId, ticketTypeId));
        const values = tickets.map((ticket, index) => `($${index * 3 + 1}, $${index * 3 + 2}, $${index * 3 + 3})`).join(',');
        const params = tickets.flatMap(ticket => [ticket.orderId, ticket.ticketTypeId, ticket.qrCode]);
